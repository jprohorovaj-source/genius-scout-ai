...
---
## Масштабирование и отказоустойчивость (Highload Architecture)

Если планируется высокая нагрузка (например, одновременная работа большого числа пользователей), текущую архитектуру рекомендуется расширить следующими компонентами:

1. **Очереди задач (Celery + Redis):** Асинхронная обработка запросов пользователей для защиты от перегрузки пула потоков.
2. **Кэширование ответов:** Сохранение результатов поиска и анализа по популярным темам для мгновенной выдачи без повторных запросов к внешним API.
3. **Микросервисная архитектура:** Вынесение тяжелых ML-моделей (эмбеддинги, реранкинг) на отдельный пул GPU-серверов.
4. **Горизонтальное масштабирование:** Упаковка веб-приложения в Docker-контейнеры и развертывание через Kubernetes с балансировщиком нагрузки.
...
##  Как запустить проект в Streamlit Cloud

Вы можете развернуть и запустить интерактивную демонстрацию **GeniusScout AI** в облаке совершенно бесплатно с помощью [Streamlit Community Cloud](https://share.streamlit.io/):

1. **Подготовьте репозиторий:** Убедитесь, что все файлы проекта (`streamlit_app.py`, `agent_core.py`, `collector.py`, `gate.py`, `vector_store.py`, `config.py`) и файл зависимостей **`requirements.txt`** загружены в ваш публичный репозиторий на GitHub.
2. **Войдите на платформу:** Перейдите на сайт [share.streamlit.io](https://share.streamlit.io/) и авторизуйтесь с помощью своей учетной записи GitHub.
3. **Создайте приложение:** 
   * Нажмите кнопку **«New app»**.
   * Выберите ваш репозиторий с проектом GeniusScout AI.
   * Укажите ветку (например, `main`) и путь к главному файлу приложения: `streamlit_app.py`.
4. **Запустите деплой:** Нажмите кнопку **«Deploy!»**. Платформа автоматически установит все необходимые библиотеки из файла `requirements.txt`.
5. **Готово!** После завершения сборки вы получите публичную ссылку на ваше работающее облачное приложение. Удачи!
